...
import { getCurrentUserOrThrow } from "@/lib/server/auth-request";
import { aiService } from "@/lib/ai/service";
import { handleAIRouteError } from "@/lib/server/route-ai-logging";
  ...
  try {
    ...
    if (!id) {
      return NextResponse.json(
        { error: "Job ID is required", requestId },
        { status: 400 }
      );
    }
    ...
    if (jobId !== id) {
      return NextResponse.json(
        { error: "Job ID mismatch", requestId },
        { status: 400 }
      );
    }
    ...
    console.error("Error in POST /api/jobs/[id]/improve-section:", error);
    return NextResponse.json(
      { error: "Failed to generate improvement", requestId },
      { status: 500 }
    );
  }
});
